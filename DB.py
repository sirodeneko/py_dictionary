...
class Sqlopen():
    ...
    def SetWord(self, word, pronounce, result):
        ...
        sql_cmd = r'insert into %s values (null,%s,%s,%s,%s,%s,%s,%s,%s)' % (
            tableName, word_f, pronounce_f[0], pronounce_f[1], result_f[0], result_f[1], result_f[2], result_f[3], result_f[4])
    ...
    # 模糊查询单词
    def FuzzyGetWord(self, word):

        tableName = 'Ewordmap'

        word_f = word

        sql_cmd = r"select name from %s where name LIKE '%s%%' limit 10" % (
            tableName, word_f)
        # 执行查询语句:
        self.cursor.execute(sql_cmd)
