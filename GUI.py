...
ResultLabel = tk.Label(top, text='Nothing', height=15,
                       width=65, bg='#FFFFDD', justify=tk.LEFT)  # 创建一个显示标签
...
def func1():
    ...
    def check(event):
        words = WordEntry.get()
        sdh = Control.Handle()
        out_string = sdh.FuzzyGetWord(words)
        ResultLabel.configure(text=out_string, anchor=tk.NW)

    WordEntry.bind('<KeyRelease>', check)
